...
export const createEmptySetupState = () => ({
    positions: { black: [], white: [] },
    ready: { black: false, white: false }
});
export const resetSetupState = (state) => {
    state.setup = createEmptySetupState();
    state.updatedAt = Date.now();
};
export const setSetupPositions = (state, color, positions) => {
...
export const setSetupReady = (state, color, ready) => {
    if (ready) {
...
export const canStartGame = (state) => {
...
export const startGameFromSetup = (state) => {
...
const createPiecesForColor = (state, color, positions) => {
...
export const beginTurn = (state, color) => {
    state.turn = color;
    state.movesTaken = 0;
    state.turnOrigins = {};
...
const validateSetupPositions = (color, positions) => {
...
export const applyMove = (state, player, command) => {
...
const recordHistory = (state, player, outcome) => {
    const entry = {
...
const simulateMove = (state, command) => {
    const piece = state.pieces[command.pieceId];
...
const resolveCaptures = (pieces, board) => {
    const captured = [];
    const pending = new Set(Object.keys(pieces).filter((id) => pieces[id].status === "active" && pieces[id].position));
...
const isCaptured = (pieces, board, piece) => {
    if (!piece.position)
        return false;
...
const commitMove = (state, outcome) => {
    ...
    for (const capturedId of outcome.captured) {
        ...
        const capturingPlayer = capturedPiece.owner === "black" ? "white" : "black";
        state.captures[capturingPlayer] += 1;
    }
};
const evaluateGameCompletion = (state) => {
...
export const countActivePieces = (state, color) => Object.values(state.pieces).filter((piece) => piece.owner === color && piece.status === "active" && piece.position).length;
export const getLegalMovesForPiece = (state, pieceId) => {
    ...
    for (let dRow = -1; dRow <= 1; dRow += 1) {
        for (let dCol = -1; dCol <= 1; dCol += 1) {
            if (dRow === 0 && dCol === 0)
                continue;
            const candidate = { row: from.row + dRow, col: from.col + dCol };
            const move = { pieceId, to: candidate };
...
const hasAnyLegalMove = (state, color) => {
    const activePieces = Object.values(state.pieces).filter((piece) => piece.owner === color && piece.status === "active" && piece.position && state.turnOrigins[piece.id]);
